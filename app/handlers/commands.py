...
from app.keyboards.keyboards import get_main_keyboard
...
async def stats(message: types.Message):
    ...
    try:
        if message.from_user.id not in current_app.config['ADMIN_IDS']:
            await message.answer("У вас нет доступа к этой команде.")
            return
        
        # Получаем статистику
        total_users = User.query.count()
        pregnant_users = User.query.filter_by(is_pregnant=True).count()
        baby_users = User.query.filter_by(is_pregnant=False).count()
